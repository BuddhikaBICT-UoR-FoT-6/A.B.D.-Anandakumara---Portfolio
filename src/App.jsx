...
const SystemCore = () => (
  <motion.div 
    ...
    className="relative w-64 h-64 border border-[#ff3300]/50 rounded-full flex items-center justify-center bg-[#0a0806]/82 backdrop-blur-sm shadow-[0_0_30px_rgba(168,85,247,0.2)] pointer-events-auto"
  >
    <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute inset-2 border border-dashed border-[#ff6600]/30 rounded-full pointer-events-none" />
    <motion.div animate={{ rotate: -360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="absolute inset-6 border border-dotted border-[#ff3300]/40 rounded-full pointer-events-none" />
    <div className="text-center pointer-events-none">
      <div className="text-xs font-mono text-[#ffaa00] mb-1 tracking-widest">SYSTEM_CORE</div>
      <div className="text-3xl font-bold bg-gradient-to-r from-[#ff6600] to-[#ffaa00] bg-clip-text text-transparent">ACTIVE</div>
      <div className="mt-2 text-[10px] font-mono text-zinc-500">
        <div>CPU_LOAD: <span className="text-[#ff6600]">24%</span></div>
        <div>MEM_ALLOC: <span className="text-[#ffaa00]">1.2GB</span></div>
        <div>CORE_TEMP: <span className="text-[#ff6600]">42°C</span></div>
      </div>
    </div>
  </motion.div>
);
...
const HoverModule = () => (
  <motion.div 
    ...
    className="w-64 border rounded-xl p-6 backdrop-blur-md transition-colors duration-500 border-[#ff6600]/50 bg-[#0a0806]/90 shadow-[0_0_30px_rgba(34,211,238,0.2)] pointer-events-auto"
  >
    <div className="flex justify-between items-center mb-4">
      <span className="text-xs font-mono text-[#ff6600] tracking-wider">ENV_SCANNER</span>
    ...
    <div className="h-1 w-full bg-zinc-800 rounded overflow-hidden mb-4">
      <motion.div 
        className="h-full bg-gradient-to-r from-[#ff3300] to-[#ff6600]"
        animate={{ x: ['-100%', '100%'] }}
...
const Typewriter = ({ strings }) => {
  ...
  return (
    <span className="text-[#ffaa00] border-r-2 border-[#ffaa00] pr-1 animate-[pulse_1s_ease-in-out_infinite]">
      {currentText}
    </span>
  );
};
...
const NavBar = ({ fullName }) => (
  <div className="fixed top-0 left-0 right-0 z-50 px-6 py-4 pointer-events-none">
    <nav className="container mx-auto pointer-events-auto flex justify-between items-center bg-[#0a0806]/82 backdrop-blur-md border border-[#ff3300]/20 rounded-xl p-4 shadow-[0_0_20px_rgba(168,85,247,0.1)]">
      <div className="text-xl font-bold font-mono text-[#ff6600] flex items-center gap-2">
        <span className="bg-gradient-to-r from-[#ff6600] to-violet-600 bg-clip-text text-transparent">{fullName}</span>
      </div>
      <div className="hidden md:flex gap-6 text-sm font-mono tracking-widest text-zinc-400">
        {['About', 'Projects', 'Skills', 'Contact'].map(item => (
          <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-[#ff6600] transition-colors">
            {item}
          </a>
        ))}
      </div>
      <div className="flex items-center gap-4 text-xs font-mono text-zinc-500 hidden sm:flex">
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 bg-[#ff6600] rounded-full animate-[pulse_1s_ease-in-out_infinite]"></span> API: ONLINE</span>
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 bg-[#ff6600] rounded-full animate-[pulse_1.5s_ease-in-out_infinite]"></span> MQTT: OK</span>
      </div>
    </nav>
  </div>
);
...
const AdminLoginModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm pointer-events-auto">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#0a0806] border border-[#ff3300]/50 p-8 rounded-xl w-full max-w-sm shadow-[0_0_50px_rgba(168,85,247,0.2)]">
        <div className="text-center mb-6">
          <div className="text-4xl mb-4">🔐</div>
          <h2 className="text-2xl font-bold text-zinc-100 font-sans">Admin Access</h2>
          <p className="text-sm text-[#ffaa00] font-mono mt-2">Authentication required</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="password" autoFocus value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password..." className="w-full bg-black/50 border border-[#ff3300]/30 rounded p-3 text-zinc-100 font-mono outline-none focus:border-[#ff6600] transition-colors pointer-events-auto" />
          <div className="flex gap-3">
            <button type="submit" className="flex-1 bg-[#cc3300] hover:bg-[#ff3300] text-white font-bold py-3 rounded transition-colors shadow-[0_0_15px_rgba(168,85,247,0.3)] pointer-events-auto">Unlock</button>
  ...
  <section id="about" className="py-24 relative z-30 pointer-events-auto">
    <div className="container mx-auto px-6">
      <div className="mb-12 pointer-events-none">
        <span className="text-[#ff6600] font-mono text-sm tracking-widest uppercase block mb-2">Who I Am</span>
      ...
      <div className="grid md:grid-cols-[1fr_2fr] lg:grid-cols-[280px_1fr_1fr] gap-8 items-start">
        <div className="flex flex-col items-center gap-6">
          {profilePhoto && (
            <div className="relative group pointer-events-auto">
              <div className="w-56 h-56 rounded-full overflow-hidden border-4 border-[#ff3300]/50 shadow-[0_0_40px_rgba(168,85,247,0.2)] transition-transform duration-500 group-hover:scale-105">
                <img src={profilePhoto} alt="Profile" className="w-full h-full object-cover" />
              </div>
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap bg-[#0a0806] border border-[#ff3300]/50 text-zinc-300 text-xs px-5 py-2.5 rounded-full font-mono flex items-center gap-2.5 shadow-[0_0_20px_rgba(168,85,247,0.3)]">
                <span className="w-2 h-2 rounded-full bg-[#ffaa00] animate-[pulse_1.5s_ease-in-out_infinite] shadow-[0_0_8px_#ffaa00]"></span>
                Open to work
              </div>
            </div>
          )}
        </div>
        <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff3300]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)] h-full pointer-events-auto">
          <h3 className="text-2xl font-bold text-[#ff6600] mb-6 font-sans">Profile</h3>
        ...
        <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff6600]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)] h-full pointer-events-auto">
          <h3 className="text-2xl font-bold text-[#ffaa00] mb-6 font-sans">Highlights</h3>
          <ul className="space-y-4 text-zinc-300 font-mono text-xs">
            {data.highlights.map((h, i) => (
              <li key={i} className="flex items-start gap-3 leading-snug">
                <span className="text-[#ff6600] mt-0.5 animate-pulse shrink-0">›</span> 
                <span>{h}</span>
              </li>
            ))}
          </ul>
  ...
  return (
    <section id="projects" className="py-24 relative z-30 pointer-events-none">
      <div className="container mx-auto px-6 pointer-events-auto">
        <div className="mb-12 pointer-events-none">
          <span className="text-[#ff3300] font-mono text-sm tracking-widest uppercase mb-2 block">My Work</span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((p, i) => (
            <motion.div 
              ...
              className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff3300]/20 hover:border-[#ff3300]/50 rounded-xl p-6 flex flex-col transition-all overflow-hidden group cursor-crosshair pointer-events-auto"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {p.badge && <span className="text-[10px] font-mono bg-red-900/40 text-[#ff6600] px-2 py-1 rounded border border-[#ff3300]/30 whitespace-nowrap">{p.badge}</span>}
              </div>
              <p className="text-zinc-400 text-sm mb-6 flex-1">{p.description || p.desc}</p>
              {p.tech && (
                <div className="flex flex-wrap gap-2 mb-6">
                  {p.tech.map(t => <span key={t} className="text-[10px] font-mono text-[#ffaa00] bg-orange-900/20 px-2 py-1 rounded-full border border-[#ff6600]/20">{t}</span>)}
                </div>
              )}
              <div className="flex gap-4 text-sm font-mono font-bold mt-auto border-t border-[#ff3300]/20 pt-4">
                {p.liveUrl && <a href={p.liveUrl} target="_blank" rel="noreferrer" className="text-[#ff6600] hover:text-[#ffaa00] transition-colors">Live ↗</a>}
                {p.codeUrl && <a href={p.codeUrl} target="_blank" rel="noreferrer" className="text-[#ff6600] hover:text-[#ffaa00] transition-colors">Code ↗</a>}
              </div>
            </motion.div>
          ))}
        </div>
...
const SkillsSection = ({ skills }) => (
  <section id="skills" className="py-24 relative z-30 pointer-events-auto">
    <div className="container mx-auto px-6">
      <div className="mb-12 pointer-events-none">
        <span className="text-[#ff6600] font-mono text-sm tracking-widest uppercase block mb-2">Toolkit</span>
        <h2 className="text-4xl md:text-5xl font-bold text-zinc-100 mt-2">Skills & Certifications</h2>
      </div>
      <div className="grid md:grid-cols-2 gap-12">
        <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff6600]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)]">
          <h3 className="text-2xl font-bold text-[#ffaa00] mb-6 font-sans">Technical Expertise</h3>
          <div className="flex flex-wrap gap-3">
            {skills.technical.map((s, i) => (
              <div key={i} className="flex items-center gap-2 bg-orange-900/20 border border-[#ff6600]/30 px-3 py-2 rounded-full text-sm text-zinc-300 font-mono">
                {s}
              </div>
            ))}
          </div>
          <h3 className="text-2xl font-bold text-[#ffaa00] mb-6 mt-10 font-sans">Soft Skills & Tools</h3>
          <div className="flex flex-wrap gap-3">
            {skills.soft.map((s, i) => (
              <div key={i} className="bg-red-900/20 border border-[#ff3300]/30 px-3 py-1.5 rounded-full text-sm text-purple-300">{s}</div>
            ))}
          </div>
        </div>
        <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff3300]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)]">
          <h3 className="text-2xl font-bold text-[#ff6600] mb-6 font-sans">Recent Certifications</h3>
          <div className="space-y-4">
            <div className="p-4 bg-red-900/20 border border-[#ff3300]/20 rounded-xl flex justify-between items-center transition-colors">
              <div>
              ...
              <span className="text-[10px] font-mono text-[#ffaa00]">Oct 2025</span>
            </div>
            <div className="p-4 bg-red-900/20 border border-[#ff3300]/20 rounded-xl flex justify-between items-center transition-colors">
              <div>
              ...
              <span className="text-[10px] font-mono text-[#ffaa00]">Sep 2025</span>
            </div>
            <div className="p-4 bg-red-900/20 border border-[#ff3300]/20 rounded-xl flex justify-between items-center transition-colors">
              <div>
            ...
            <div className="p-4 bg-red-900/20 border border-[#ff3300]/20 rounded-xl flex justify-between items-center transition-colors">
              ...
              <span className="text-[10px] font-mono text-[#ffaa00]">Jan 2025</span>
            </div>
          </div>
          <details className="mt-6 group border border-[#ff3300]/20 rounded-lg overflow-hidden pointer-events-auto">
            <summary className="p-4 bg-red-900/10 cursor-pointer font-bold text-[#ff6600] flex justify-between items-center outline-none">
              Additional & Earlier Certifications
              <span className="text-xs opacity-50 group-open:rotate-180 transition-transform">▼</span>
            </summary>
            <div className="p-4 space-y-3 bg-black/30 border-t border-[#ff3300]/20">
...
const ContactSection = ({ email, linkedin, github }) => {
  ...
  return (
    <section id="contact" className="py-24 relative z-30 pointer-events-none">
      <div className="container mx-auto px-6">
        <div className="mb-12 pointer-events-none">
          <span className="text-[#ff3300] font-mono text-sm tracking-widest uppercase mb-2 block">Let's Talk</span>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff3300]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)] pointer-events-auto">
            ...
            <form action={`mailto:${email}`} method="GET" className="space-y-4">
              <div>
                <label className="block text-sm text-[#ff6600] mb-1">Name / Subject</label>
                <input name="subject" placeholder="Your name / subject" required className="w-full bg-black/50 border border-[#ff3300]/30 rounded p-3 text-zinc-100 outline-none focus:border-[#ff6600] transition-colors" />
              </div>
              <div>
                <label className="block text-sm text-[#ff6600] mb-1">Message</label>
                <textarea name="body" rows="5" placeholder="What would you like to talk about?" required className="w-full bg-black/50 border border-[#ff3300]/30 rounded p-3 text-zinc-100 outline-none focus:border-[#ff6600] transition-colors"></textarea>
              </div>
              <button type="submit" className="w-full bg-[#cc3300] rounded font-bold py-3 text-lg hover:bg-[#ff3300] transition-colors shadow-[0_0_15px_rgba(168,85,247,0.3)]">
                Send Message →
              </button>
          ...
          <div className="bg-[#0a0806]/82 backdrop-blur-md border border-[#ff6600]/30 rounded-xl p-8 shadow-[0_0_20px_rgba(255,40,0,0.08),inset_0_0_40px_rgba(0,0,0,0.4)] pointer-events-auto h-fit">
            <h3 className="text-2xl font-bold text-zinc-100 mb-6">Reach Me Directly</h3>
            <ul className="space-y-6">
              <li className="flex items-center gap-4">
                <span className="text-2xl">✉</span>
                <div className="flex-1">
                  <a href={`mailto:${email}`} className="text-[#ffaa00] hover:text-cyan-300 hover:underline">{email}</a>
                </div>
                <button onClick={handleCopyEmail} className="p-2 bg-red-900/30 border border-[#ff3300]/30 rounded hover:bg-red-900/60 transition-colors" title="Copy Email">
                  📋
                </button>
              </li>
              <li className="flex items-center gap-4">
                <span className="text-2xl">⎔</span>
                <a href={github} target="_blank" rel="noreferrer" className="text-[#ffaa00] hover:text-cyan-300 hover:underline">github.com/BuddhikaBICT-UoR-FoT-6</a>
              </li>
              <li className="flex items-center gap-4">
                <span className="text-2xl">💼</span>
                <a href={linkedin} target="_blank" rel="noreferrer" className="text-[#ffaa00] hover:text-cyan-300 hover:underline">linkedin.com/in/buddhika-darshan</a>
              </li>
            </ul>
          </div>
        </div>
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-transparent font-sans text-zinc-100 overflow-x-hidden selection:bg-[#ff3300]/30">
      
      {/* Background WebGL Canvas */}
      <div className="fixed inset-0 z-0 pointer-events-auto bg-[#1a1e24]">
      ...
      <div className="relative z-10 pointer-events-none container mx-auto px-6 py-4 flex flex-col min-h-screen">
        ...
        <div className="flex-1 grid lg:grid-cols-2 gap-12 items-center pt-32 pb-24 pointer-events-none">
          <div className="space-y-8">
            <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.8 }}>
              <span className="text-[#ff6600] font-mono text-sm tracking-widest uppercase mb-4 block">
            ...
            <div className="flex gap-4 pointer-events-none">
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }} 
                className="px-8 py-4 bg-[#cc3300] rounded-full font-bold text-lg shadow-[0_0_15px_rgba(168,85,247,0.3)] transition-all pointer-events-auto"
              >
                View Projects →
              </motion.button>
              {resumeData && (
                <motion.a 
                  ...
                  className="inline-flex items-center px-8 py-4 border border-[#ff6600]/30 text-[#ffaa00] rounded-full font-bold transition-colors pointer-events-auto"
                >
                  ↓ Download CV
                </motion.a>
              )}
        ...
        <footer className="mt-auto pt-12 flex justify-between items-center text-[10px] font-mono text-zinc-500 tracking-[0.3em] uppercase bg-[#0a0806]/82 p-4 rounded-t-xl border-t border-[#ff3300]/30 pointer-events-auto">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-[#ff6600] rounded-full animate-pulse shadow-[0_0_8px_#22d3ee]" />
            System Operational
          </div>
